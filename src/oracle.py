...
from online_properties import OnlineBatch, StopStatus
from features import PairFeatureBuilder, PairFeatures
from judges import JudgeLabel, JudgeRequest, JudgeResult, JudgedPairExample, JudgeTrainingStore, SemanticReuseJudge
from online_properties import OnlineUpdater
...
from thresholds import ThresholdCalibrationRequest, ThresholdProvider, ThresholdScope
...
class TrainableSemanticCacheOracle(SemanticCacheOracle):
    ...
    def _fit_in_current_thread(self, split: TrainCalibEvalSplit) -> None:
        from semantic_types import LabeledPairBatch
    ...
    def _monitor_fpr_upper_bound_locked(self) -> float | None:
        n = int(self._monitor_h0_count)
        if n <= 0:
            return None
        z = 1.96
        phat = float(self._monitor_false_accepts) / float(n)
        denom = 1.0 + (z * z) / n
        center = phat + (z * z) / (2.0 * n)
        radius = z * sqrt((phat * (1.0 - phat) / n) + ((z * z) / (4.0 * n * n)))
